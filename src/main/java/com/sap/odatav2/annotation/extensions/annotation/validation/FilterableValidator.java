...
public class FilterableValidator implements AnnotationValidator {
	public FilterableValidator() {
	}
	...
	public void operationValidation(ValidatorParameters params)
		...
		if (this.requestMethodIsRelevantForFieldValidation(params.getODataHttpMethod()) && CollectionUtils
				.allNonNull(new Object[] { params.getEdmEntitySet(), params.getFilterExpression() })) {
			Optional.ofNullable(ValidatorUtils.getCommonProperties(params.getFilterExpression()))
					.ifPresent((filterFieldNames) -> {
						try {
							ValidatorUtils.checkFilterableSortableProps(params.getEdmEntitySet(), filterFieldNames,
									AnnotationPropertyNames.Filterable);
